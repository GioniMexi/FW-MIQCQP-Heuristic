...
{
	// add all propagators to the queue
	for (PropagatorPtr p: propagators)
		queue.push_back(p->getID());
...
static constexpr int MAX_PROP_COUNT = 50;
...
void PropagationEngine::tightenedLb(int j, double newValue, double oldValue)
{
	...
	if (domain->isVarFixed(j)) lastFixed.push_back(j);
