...
function run_heuristics(tree, x, heuristic_list; rng=Random.GLOBAL_RNG)
    ...
    heuristic_lmo = TimeTrackingLMO(inner_lmo, tree.root.problem.integer_variables, tree.root.problem.tlmo.time_ref, tree.root.problem.tlmo.time_limit, tree.root.problem.tlmo.max_time_lmo)
        ...
        if flip_coin(heuristic.prob, rng)
            ...
            if !isempty(list_x_heu)
                ...
                for (i, x_heu) in enumerate(list_x_heu)
                    feasible = check_feasibility ? is_integer_feasible(tree, x_heu) && tree.root.options[:domain_oracle](x_heu) && is_linear_feasible(tree.root.problem.tlmo, x_heu) : true
                    if feasible
...
function probability_rounding(tree::Bonobo.BnBTree, tlmo::Boscia.TimeTrackingLMO, x; rng=Random.GLOBAL_RNG)
    ...
    if status == MOI.INFEASIBLE || status == MOI.DUAL_INFEASIBLE || status == INFEASIBLE || status == UNBOUNDED
