...
time_ref = Dates.now()
    ...
    time_lmo = TimeTrackingLMO(blmo, integer_variables, time_ref, time_limit, max_time_lmo)
    ...
    if start_solution !== nothing
        ...
        node = tree.nodes[1]
        _, _, _, _ = tree.root.options[:post_heuristics_callback](tree, node, v, :initial)
    end
    ...
    run_heuristics(tree, v, tree.root.options[:heuristics])
